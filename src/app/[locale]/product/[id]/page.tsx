...
export default function ProductPage() {
  ...
  return <main className="min-h-screen bg-[color:var(--surface)] text-[color:var(--foreground)] font-antonio"><Header /><div className="container mx-auto px-6 pt-32 pb-24"><div className="grid lg:grid-cols-2 gap-16">
    <section><div className="aspect-[3/4] rounded-2xl overflow-hidden bg-[color:var(--surface-muted)] relative">{images[imageIndex] ? (
  <button
    type="button"
    onClick={() => window.open(images[imageIndex].url, '_blank', 'noopener,noreferrer')}
...
)}{images.length > 1 && <><button aria-label="Poprzednie zdjęcie" onClick={() => setImageIndex((current) => (current - 1 + images.length) % images.length)} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/80 text-black"><ChevronLeft /></button><button aria-label="Następne zdjęcie" onClick={() => setImageIndex((current) => (current + 1) % images.length)} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-full bg-white/80 text-black"><ChevronRight /></button></>}</div></section>
