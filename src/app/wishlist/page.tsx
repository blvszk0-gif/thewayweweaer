...
import { Heart, Bell, ShoppingBag, Trash2, ArrowRight, X } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredItems.map((item) => (
              <motion.div
                ...
                whileHover={{ y: -10 }}
                className="bg-white rounded-[40px] overflow-hidden shadow-2xl border border-white/20 group flex flex-col"
              >
                <div className="aspect-[4/5] relative overflow-hidden bg-black/5 group-mini-slider">
                  <div className="flex overflow-x-auto snap-x snap-mandatory h-full no-scrollbar">
                    {item.images.map((img, idx) => (
                      <div key={idx} className="min-w-full h-full snap-center">
                        <img src={img} alt={item.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
                      </div>
                    ))}
                  </div>
                ...
                <div className="p-8 flex-1 flex flex-col">
                   ...
                   <Link href={`/product/${item.id}`}>
                    <h3 className="text-xl font-black uppercase tracking-tighter italic mb-4 hover:opacity-60 transition-opacity leading-none">{item.name}</h3>
                   </Link>
